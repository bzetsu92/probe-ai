...
export function ArchitectureSection() {
  ...
  return (
    <section id="architecture" className="py-28" style={{ background: "var(--surface)" }}>
      <div className="max-w-5xl mx-auto px-10" ref={ref as React.Ref<HTMLDivElement>}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
          ...
          >
            ...
            <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight leading-tight mb-4" style={{ fontFamily: "var(--font-syne)" }}>
          ...
          >
            {layers.map((layer, i) => (
              <motion.div
                ...
                className="flex items-center gap-4 px-4 py-3.5 rounded-lg mb-1 group cursor-default transition-colors duration-200"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <span className="block text-base font-semibold" style={{ color: "var(--text)" }}>{layer.name}</span>
                  <span className="block text-sm mt-0.5" style={{ fontFamily: "var(--font-mono)", color: "var(--text-dim)" }}>{layer.tech}</span>
                </div>
              </motion.div>
            ))}
          </motion.div>
